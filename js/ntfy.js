(function(){
  ...
  async function publish(command,messageId){
    ...
    try{
      const response=await fetch(`${server}/${cfg.ntfyTopic}`,{method:"POST",headers:{"Content-Type":"text/plain; charset=utf-8"},body,signal:controller.signal,cache:"no-store",referrerPolicy:"no-referrer"});
