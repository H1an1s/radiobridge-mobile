(function(){
  ...
  async function anonymousAuth(){
    ...
    idToken=data.idToken||""; refreshToken=data.refreshToken||""; expiresAt=Date.now()+Math.max(60,Number(data.expiresIn||3600)-60)*1000;
    if(!idToken) throw new Error("Firebase Auth не вернул токен");
    return idToken;
  }
  async function token(){
    if(idToken && Date.now()<expiresAt) return idToken;
    if(!authPromise) authPromise=anonymousAuth().finally(()=>{authPromise=null});
    return authPromise;
  }
